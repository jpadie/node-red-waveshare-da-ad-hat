...
import { WaveshareADProperties, PythonScriptResult } from '../../types/node-red';

export function createWaveshareADNode(RED: any) {
  return function WaveshareADNode(this: any, config: WaveshareADProperties) {
    RED.nodes.createNode(this, config);
    
    const node = this;
    let readTimer: NodeJS.Timeout | undefined;
    
    // Set up auto-read if enabled
    if (config.autoRead && config.readInterval > 0) {
      setupAutoRead(config.readInterval);
    }
    ...
    async function handleInput(msg: any, send: (msg: any) => void, done: () => void): Promise<void> {
      try {
        const channel = msg.payload?.channel ?? config.channel ?? 0;
        const gain = msg.payload?.gain ?? config.gain ?? 1;
        const buffered = msg.payload?.buffered ?? config.buffered ?? false;
        const dataRate = msg.payload?.dataRate ?? config.dataRate ?? 1000;
        
        if (typeof channel !== 'number' || channel < 0 || channel > 7) {
          throw new Error('Channel must be a number between 0 and 7 (single-ended)');
        }
        ...
        node.error(`ADC Error: ${error}`, msg);
        done();
      }
    }
    ...
    function setupAutoRead(interval: number): void {
